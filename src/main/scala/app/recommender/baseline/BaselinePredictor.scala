...
class BaselinePredictor() extends Serializable {
  ...
  def init(ratingsRDD: RDD[(Int, Int, Option[Double], Double, Int)]): Unit = {
    ...
    val temp = ratingsRDD.map{case(user_id, movie_id, old_rating, rating, timestamp) => (user_id, (movie_id, rating))}
      .join(user_avg_ratings).map{case(user_id, ((movie_id,rating), user_avg)) => (user_id, movie_id, rating, user_avg)}
      .map{case(user_id, movie_id, rating, user_avg)=>
  ...
  def predict(userId: Int, movieId: Int): Double = {
    ...
    val global_average = state.map{case(user_id, movie_id, rating, user_avg, user_avg_dev) =>
      ((user_id, movie_id), (rating, 1))}.reduceByKey((x, y) => (x._1 + y._1, x._2 + y._2))
    ...
    else{p = user_avg + movie_avg * Scale((user_avg + movie_avg), user_avg)}
